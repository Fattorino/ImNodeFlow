...
// #define ConnectionFilter_Numbers    [](ImFlow::Pin* out, ImFlow::Pin* in){ return out->getDataType() == typeid(double) || out->getDataType() ==
// typeid(float) || out->getDataType() == typeid(int); }
...
namespace ImFlow
{
	...
	public:
		PinStyle(
			ImU32 color, int socket_shape, float socket_radius, float socket_hovered_radius, float socket_connected_radius, float socket_thickness)
			: color(color),
	...
	public:
		...
		{
			return std::make_shared<NodeStyle>(IM_COL32(191, 134, 90, 255), ImColor(233, 241, 244, 255), 6.5f);
		}
	};
	...
	public:
		 ...
		 */
		explicit ImNodeFlow(std::string name)
	...
	public:
		 ...
		 */
		void droppedLinkPopUpContent(std::function<void(Pin *dragged)> content, ImGuiKey key = ImGuiKey_None)
		{
	...
	private:
		...
		ImSelectionRect m_selection_rect;
		bool m_using_selection_rect = false;
	};
	...
	public:
		...
		addIN(const std::string &name, T defReturn, std::function<bool(Pin *, Pin *)> filter, std::shared_ptr<PinStyle> style = nullptr);
		...
		std::shared_ptr<InPin<T>> addIN_uid(
			const U &uid, const std::string &name, T defReturn, std::function<bool(Pin *, Pin *)> filter, std::shared_ptr<PinStyle> style = nullptr);
	...
	public:
		...
		{
			return [](Pin *out, Pin *in) { return out->getDataType() == in->getDataType(); };
		}
		static std::function<bool(Pin *, Pin *)> Numbers()
		{
			return [](Pin *out, Pin *in)
			{ return out->getDataType() == typeid(double) || out->getDataType() == typeid(float) || out->getDataType() == typeid(int); };
		}
	};
	...
	public:
		 ...
		 */
		explicit InPin(
			PinUID uid, const std::string &name, T defReturn, std::function<bool(Pin *, Pin *)> filter, std::shared_ptr<PinStyle> style,
